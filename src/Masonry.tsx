...
const Masonry = ({
  ...
  useEffect(() => {
    const imageUrls = items.map(i => i.img).filter(Boolean) as string[];
    ...
    <div ref={containerRef} className="masonry-list">
      {grid.map(item => {
        return (
          ...
          >
            <div 
              className={`masonry-item-content ${item.content ? 'has-text' : ''}`} 
              style={{ 
                ...(item.img ? { backgroundImage: `url(${item.img})` } : {}),
              }}
            >
              {item.content && <div className="text-content-inner">{item.content}</div>}
              {colorShiftOnHover && (
                <div
